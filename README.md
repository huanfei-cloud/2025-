...
本测试题用于北京航空航天大学机甲大师Transistor战队电控组的入队考核过程，我们希望更多的同学加入我们队伍，所以我们的题目分为“基础篇”和“进阶篇”，基础部分的题目基于**培训内容**和**平时所学内容**，进阶部分的题目难度将会**略微**超过本科期间的**正常**知识范畴，两部分都需要大家尽力完成。
...
**特别强调**：**电控控制和电控硬件提交测试答案时需要分开提交（git提交除外）**

## 请注意：

1. 考核为期**2周**，每周都会发放一定数量的题目，并要求在**当周内提交**，提交时间将在上一周末与答题卡同步在群内下发。
2. 每个组别基础部分的题目和进阶部分的题目只是区分难度，各位都要尽力完成。
3. 提交方法包括**上传至北航云盘**、**使用github提交**等，不同考核题的提交方法会在每周任务发布的同时公布。请大家按照规定格式提交，以方便阅卷。
...
### github基本操作（电控控制/电控硬件）

**任务简介**

- Git是一个开源的分布式版本控制系统，用于敏捷高效地处理任何或小或大的项目,可以帮助你进行自动的版本管理。GitHub 是一个基于云的代码托管和协作平台，控制组之后的线上测试都是通过github发布，在多人协作的情况下，使用github就会相当的方便，能够有效地协调多位开发者对不同地方的改动。
...
- 在想要克隆本仓库的地方，右键菜单点击“**`Git Bash Here`**”，输入相关代码，即可实现将远程仓库克隆到本地的操作，注意**之后的任务会放置在相关任务文件夹中**（**参考教程里会讲到，学习即可**）
- 具体任务
...
在嘉立创上下载并阅读STM32F334R8T6芯片手册，以LQFP64型号回答以下问题：
...
4. 请简要说明VBAT，VSSA，VDDA，VSS，VDD引脚功能及区别

5. 请简要说明NRST，BOOT0引脚作用

### CubeMX使用和CAN通信基础（电控控制）
...
- 请简要说明电阻、电容、二极管、三极管的作用及一些特殊用法，如上（下）拉电阻、采样电阻、0欧电阻等；

### 电源电路基础（电控硬件）

**任务要求**

- 请简要介绍常见电源电路，如BUCK电路，BOOST电路，BUCK-BOOST电路等，包括工作原理、优缺点及原理图。
...
## 各任务提交方式
### 整体提交形式
  - git提交+除git提交外各任务分别有一个整体文件夹放在一个总文件夹里（文件夹的命名**{学号}{姓名}{组别}第一周测试任务**），最后文件夹交到[北航云盘](https://bhpan.buaa.edu.cn/link/AA9A9A5FBB39684EBA8D4F2BC0CC732457)里。
  - 示例见[北航云盘](https://bhpan.buaa.edu.cn/link/AA9A9A5FBB39684EBA8D4F2BC0CC732457)。
### 各部分具体提交形式
  - **github基本操作**：**git提交**【具体提交要求在任务要求里查看】。
  - **C/C++语言基础**：一个名为“C或C++语言基础”的文件夹里放 （文件形式的程序本身）和（照片形式的程序运行结果截图）
  - **嵌入式基础**：一个名为“嵌入式基础”的文件夹放 （word文档形式的答案）
  - **CubeMX使用和CAN通信基础**：1. **任务一**（git提交）【具体要求在其任务里查看】
                                2. **任务二**  一个名为“CubeMX使用和CAN通信基础”的文件夹放 （word文档形式的优点1、3题答案）和（2题工程文件【不用压缩】）。
  - **电气元件基础**：一个名为“电气元件基础”的文件夹放（word文档形式的答案）。
  - **电源电路基础**：一个名为“电源电路基础”的文件夹放（word文档形式的答案）。
  - **线性降压电路设计**： 一个名为“线性降压电路设计”的文件夹放（word文档形式的3、4题的答案）和（一个工程文件的包含PCB原理图、用到的元件以及说明文档【不用压缩】）。
...
$$
  ...
  2. 在1的基础上，根据注释提示补全代码[ChassisMotion](https://kdocs.cn/l/cqBQL2EeYXPr)，实现四轮麦克纳姆轮底盘各轮所需转速的解算任务.

### FreeRTOS基础（电控控制）

**任务要求**

RoboMaster的赛场上，机器人同时处理的任务、同时传递的消息繁多，这时候继续采用前后端的编程模式和全局变量的消息形式，就存在一定的局限性，不利于机器人控制逻辑的实现和代码的可维护性。这时候就需要使用嵌入式操作系统。在Transistor战队中，采用的嵌入式操作系统为FreeRTOS，几乎所有电控组的成员都需要掌握FreeRTOS的使用方法。请自行查阅相关资料，回答下列问题：

  1. 请简要说明为什么要使用嵌入式操作系统？FreeRTOS的实时性是如何体现的，又是如何实现任务调度的？
  2. 在裸机开发过程中，不同任务间的信息传递通常采用全局变量的方式。在实时操作系统中，引入了信号量的概念，请简要介绍信号量的概念以及信号量、消息队列、全局变量之间的区别。回答为什么在FreeRTOS中需要使用信号量实现任务间的通信。
  3. 实现[RoboMaster开发板C板嵌入式软件教程文档](https://kdocs.cn/l/cgz2Zw96kKbw)第十五章教程内容
...
使用Altium Designer设计一个基于STM32F407ZGT6芯片的**最小系统板**，要求工程文件中必须包含原理图、layout以及一个简要的说明文档。

**说明文档要求**

必要部分：总体介绍、各设计部分的具体介绍、使用方法/须知（比如某个按键的作用、输入输出接口的位置等）、注意事项（optional）

目的是让使用者可以快速上手，所以需要一定的**可读性**。

**详细要求**

1. 板子尺寸控制在**60*60mm**，四角需要做圆角处理（**半径3mm**），同时需要在四角打上**3.2mm直径**的过孔（**外直径为5mm**）用于固定板子，圆心距离边缘**3mm**；
2. 电源输入为**5V**，使用**xt30**接口，注意正负极对应的引脚；
3. 单片机电压为**3.3V**，因此需要添加一个**AMS1117**芯片的降压模块（入队考核第一周的进阶题）
4. 调试接口使用**gh1.25**端口（4引脚），使用**SWD**模式，注意需要对接口引脚进行标注；
5. 晶振使用一个高速**8MHz**即可，最好使用贴片型

**注意事项**	

- 先对**最小系统板**的定义、介绍进行了解，明确需要设计的电路都有什么；
- 网上的成熟设计很多，从中选取你认为最清晰最好的，学习设计思路；
- 这次任务重在提高AD软件的熟练程度，很多步骤都需要认真完成，尽可能熟悉布局、走线、铺铜等整个设计板子的流程；
- 禁止使用**自动布线**
...
$$
图2.3 在Keil中使用Arm Compiler 6以实现对C++14及更高版本的支持
$$
...
DC-DC BUCK是一种使用频率非常高的电路拓扑结构，BUCK电路是一种输入电压大于输出电压的非隔离直流变换器，常用于电源降压处理。请自行查阅相关材料，完成下列问题：

1. 什么是伏秒平衡？
2. 总结BUCK的原理（拓扑结构）；
3. 参考Buck DC-DC 的PCB layout，完成BUCK电路的原理图设计与PCB设计，使用TPS5430芯片，满足输入DC24V，输出DC5V；
4. 总结BUCK电路中电容与电感的作用。

## 各任务提交方式
### 整体提交形式
  - git提交+除git提交外各任务分别有一个整体文件夹放在一个总文件夹里（文件夹的命名**{学号}{姓名}{组别}第二周测试任务**），最后压缩成压缩包上传到[北航云盘](https://bhpan.buaa.edu.cn/link/AA9A9A5FBB39684EBA8D4F2BC0CC732457)里。
  - 示例见[北航云盘](https://bhpan.buaa.edu.cn/link/AA9A9A5FBB39684EBA8D4F2BC0CC732457)。
### 各部分具体提交形式
  - **底盘运动学结算**：一个名为“底盘运动学结算”的文件夹里放 （word文档形式的1题答案）和（代码文件【不用压缩】）
  - **FreeRTOS基础**：一个名为“FreeRTOS基础”的文件夹里放 （word文档形式的1、2题答案）和（第3题的工程文件）
  - **自动化控制原理基础**：一个名为“自动化控制原理基础”的文件夹放 （word文档形式的答案）
  - **硬件综合任务**：一个名为“硬件综合任务”的文件夹放 （一个工程文件包含原理图、layout以及一个简要的说明文档【不用压缩】）
  - **PID算法的实现**：一个名为“PID算法的实现”的文件夹放（word文档形式的1题答案）和（一个工程文件【不用压缩】）。
  - **倒立摆小车**：一个名为“倒立摆小车”的文件夹放（word文档形式的思考答案）。
  - **BUCK电路设计**： 一个名为“线性降压电路设计”的文件夹放（word文档形式的1、2、4题的答案）和（一个工程文件包含PCB原理图、用到的元件以及说明文档）。
